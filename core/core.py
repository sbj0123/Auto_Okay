...
class AutoOk:
    ...
    def get_waveform(self, case):
        ...
        if case == 'pitch':
            file_path = os.path.join(settings.MEDIA_ROOT, 'pitch_shifted.wav')
            y, sr = librosa.load(file_path)
            self.image_path = 'pitch_image.png'
        if case == 'pitch_bgm':
            file_path = os.path.join(settings.MEDIA_ROOT, 'FINAL_SONG.mp3')
            y, sr = librosa.load(file_path)
            self.image_path = 'pitch_bgm_image.png'
    ...
    def final_song(self):
        # 첫 음원 불러오기
        y, sr = librosa.load(self.url_bgm)
        sf.write("media/BGM.wav", y, sr)
        _, sr = librosa.load(self.url_vocal)
        sf.write("media/VOCAL.wav", self.list_shifted_vocal, sr)

        combined = AudioSegment.from_file("media/VOCAL.wav")
        new_audio = AudioSegment.from_file("media/BGM.wav")
        combined = combined.overlay(new_audio)

        # 합쳐진 음원 다운로드
        mp3_path = os.path.join(settings.MEDIA_ROOT, 'FINAL_SONG.mp3')
